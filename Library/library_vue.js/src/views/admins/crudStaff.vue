...
export default {
    ...
    computed: {
        DisplayUsers() {
            // return this.searchQuery ? this.searchResults : this.users;

            const usersToDisplay = this.searchQuery
                ? this.searchResults
                : this.users;
            const startIndex = (this.currentPage - 1) * this.usersPerPage;
            return usersToDisplay.slice(startIndex, startIndex + this.usersPerPage);
        },

        totalPages() {
            const totalusers = this.searchQuery
    ...
    methods: {
        ...
        search() {
            // Make a request to your backend API for searching users
            axios
                .get(`http://localhost:8000/api/admin/searchUser/${this.searchQuery}`)
                .then((response) => {
                    if (response.data.length > 0) {
                        ...
                        this.errormessage =
                            "No results found for User of : " + this.searchQuery;
                    }
                })
                .catch((error) => {
        ...
        uploadStaff() {
            ...
            axios
                .post("http://localhost:8000/api/admin/register", formData)
                .then((response) => {
                    // Handle success
        ...
        async deleteStaff(staffId) {
            try {
                await axios.delete(
                    `http://localhost:8000/api/admin/delete_user/${staffId}`
                );
                this.$router.go(0);
            } catch (error) {
        ...
        viewStaffDetail(index) {
            this.selectedStaff = this.users[index];
        },

        handleUpdateClick(users) {
            // Set the form fields based on the selected users
        ...
        updateStaff() {
            ...
            if (this.image && this.image !== this.selectedStaff.image) {
                formData.append("image", this.image);
            }
            console.log("Updating Event with ID:", this.selectedStaff.id);
            console.log("Data being sent to server:", formData);

            axios
                .post(
                    `http://localhost:8000/api/admin/update_user/${this.selectedStaff.id}`,
                    formData
                )
                .then((response) => {
                    console.log("Update successful:", response.data);

                    this.$emit("updateStaff", this.selectedStaff.id);
                    this.$router.go(0);
                })
                .catch((error) => {
            ...
            <main class="main flex flex-col flex-grow -ml-64 md:ml-0 transition-all duration-150 ease-in bg-gray-200">
                    ...
                    <div class="header-content flex items-center flex-row mx-3">
                        <div class="max-w-2xl mx-auto w-50">
                            ...
                            <div class="relative flex justify-center items-center">
                                <input type="search" id="default-search" v-model="searchQuery"
                                    ...
                                    placeholder="Search users by name username email " required />
                                <button @click="search"
                        ...
                        <div class="flex ml-auto" v-if="user">
                            <a href class="flex flex-row items-center">
                                <img :src="getUserImageURL(user)" :alt="image" class="rounded-circle img-fluid mx-2"
                                    style="width: 50px; height: 50px" />

                                <span class="flex flex-col">
                                    <span class="truncate font-semibold tracking-wide leading-none">{{ user.first_name }} {{
                                        user.last_name }}</span>
                                    <span class="truncate text-gray-500 text-xs leading-none mt-1">{{
                                        user.role_user.role_name }}</span>
                                </span>
                            </a>
                        </div>
                    </div>
                ...
                <div class="main-content flex flex-col flex-grow p-1">
                    <div class="flex justify-between">
                        ...
                        <div class="text-right mx-5 mt-2">
                            ...
                            <div class="modal fade" id="exampleModal1" tabindex="-1" aria-labelledby="exampleModalLabel"
                                aria-hidden="true">
                                <form @submit.prevent="uploadSatff">
                                                    ...
                                                    <div
                                                        ...
                                                        <div class="flex m-auto justify-around">
                                                            <div class="flex items-center justify-center w-50">
                                                                <div class="relative">
                                                                    ...
                                                                    <label for="username"
                                                                        class="absolute left-0 top-1 cursor-text peer-focus:text-xs peer-focus:-top-4 transition-all peer-focus:text-blue-700 fw-bold text-[#243673]">First
                                                                        name</label>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="flex items-center justify-center w-50">
                                                                <div class="relative">
                                                                    <input id="username" name="username" type="text"
                                                                        ...
                                                                        class="border-b border-gray-300 py-1 focus:border-b-2 focus:border-blue-700 transition-colors focus:outline-none peer bg-inherit" />
                                                                    <label for="username"
                                                                        class="absolute left-0 top-1 cursor-text peer-focus:text-xs peer-focus:-top-4 transition-all peer-focus:text-blue-700 fw-bold text-[#243673]">Last
                                                                        name</label>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="flex m-auto justify-around">
                                                            <div class="flex items-center justify-center w-50">
                                                                <div class="relative">
                                                                    ...
                                                                    <label for="username"
                                                                        class="absolute left-0 top-1 cursor-text peer-focus:text-xs peer-focus:-top-4 transition-all peer-focus:text-blue-700 fw-bold text-[#243673]">
                                                                        Username</label>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div class="flex m-auto justify-around">
                                                            <div class="flex items-center justify-center w-50">
                                                                ...
                                                                <label for="fileGenre"
                                                                    class=" fw-bold text-[#243673]">Gender</label>
                                                                <select v-model="gender" id="fileGenre"
                                                                    class="custom-select  border-bottom  bg-transparent">
                                                                    <option value="null">
                                                                        Select a gender...
                                                                    </option>
                                                        ...
                                                        <div class="flex justify-around m-auto">
                                                            ...
                                                            <div
                                                                ...
                                                                <span class=" text-dark fs-5 fw-bold">Select your Staff
                                                                    picture</span>
                                        ...
                                        <div
                                            ...
                                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                <thead class="bg-[#c5ede0] dark:bg-gray-800">
                                                    <tr>
                                                        <th scope="col"
                                                            class="px-4 py-3.5 text-sm text-left rtl:text-right">
                                                            ID
                                                        </th>
                                                        ...
                                                        <th scope="col ">
                                                            <span
                                                                class="mx-5 px-4 py-3.5 text-sm text-left rtl:text-right">Actions</span>
                                                        </th>

                                                        <th scope="col ">
                                                            <span class="px-4 py-3.5 text-sm text-left rtl:text-right">View
                                                                Detail</span>
                                                        </th>
                                                    </tr>
                                                </thead>

                                                <tbody v-for="(users, index) in DisplayUsers" :key="users.id"
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                            <div class="flex items-center gap-x-6">
                                                                ...
                                                                <div class="modal fade" id="exampleModalUpdate"
                                                                    ...
                                                                    aria-hidden="true" v-if="selectedStaff">
                                                                    <form @submit.prevent="updateStaff">
                                                                        <div class="modal-dialog modal-lg">
                                                                            <div class="modal-content">
                                                                                ...
                                                                                <div class="modal-body">
                                                                                    <div
                                                                                        class="mx-5 py-2 dashboard_textBox">
                                                                                        <div
                                                                                            class="dashboard_textBox w-100 p-2 text-center fs-6 m-auto py-4 h-auto rounded-3">
                                                                                            <div
                                                                                                class="flex m-auto justify-around">
                                                                                                <div
                                                                                            ...
                                                                                            <div
                                                                                                ...
                                                                                                <div
                                                                                                    class="formUploadImg bg-white rounded-4 text-center fs-6 border-0 m-auto py-4">
                                                                                                    <span
                                                                                                        class="lead text-dark fs-5 fw-bold">Select
                                                                                                        your Staff
                                                                                                        picture</span>
                                                                                                    <label for="file-input"
                                                                                                        class="drop-container position-relative d-flex flex-column align-items-center justify-content-center p-2 mt-3 rounded-4 border border-dashed text-muted cursor-pointer gap-2">
                                                                                                        <span
                                                                                                            class="lead drop-title text-muted fs-4 text-center">Drop
                                                                                                            here !!</span>
                                                                                                        or
                                                                                ...
                                                                                <div class="modal-footer mx-5">
                                                                                    <button type="submit"
                                                                                        class="lead btn btn-success mt-3 ml-96">
                                                                                        Update
                                                                                    </button>
                                                                                </div>
                                                                            </div>
                                                                ...
                                                                <button @click="deleteStaff(event.id)"
                                                        ...
                                                        <td class="px-4 py-2 text-sm fw-bold cursor-pointer text-gray-500 dark:text-gray-300 whitespace-nowrap"
                                                            data-bs-toggle="modal" data-bs-target="#exampleModal3"
                                                            @click="viewStaffDetail(index)">
                                                                    ...
                                                                    <div class="modal-body" v-if="selectedStaff">
                                                                        <div class="flex justify-around items-center">
                                                                            <div class="Event-card flex justify-between">
                                                                                <img :src="getStaffImgUrl(selectedStaff)"
                                                                                    class="Event-cover w-[300px] h-[300px] img-fluid"
                                                                                    alt="Event Cover" />
                                                                                <div class="Event-info mx-4">
                                                                                    ...
                                                                                    <p class="text-gray-700">
                                                                                        Role:
                                                                                        {{ selectedStaff.role_user.role_name
                                                                                        }}
                                                                                    </p>
                                                                                </div>
                                                                            </div>
                    ...
                    <div v-if="errormessage"
                        class="lead m-auto text-center fs-6 bg-[#a4c7da] w-75 rounded-5 shadow-md py-2">
                        {{ errormessage }} &#128142;
